<?php

add_translation('create_module', 'Modul erstellen');
add_translation('edit_module', 'Modul bearbeiten');
add_translation('cancel', 'Abbrechen');
add_translation('source', 'Quelle');
add_translation('embeddable', 'Einbindbar');
add_translation('shy', 'Modul verstecken');
add_translation('main_class', 'Hauptklasse');
add_translation('create_post_install_script', 'post-install Script erstellen');
add_translation('hooks', 'Ereignisse');
add_translation('module_folder', 'Modulordner');
add_translation('the_class_x_already_exists', 'Die Klasse "%class%" existiert bereits.');
